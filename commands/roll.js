...
exports.runCommand = function(user, args, msgo, DiscordMonies) {
  if (DiscordMonies.Players[user.id].needsToRoll==true) {
    ...
    embed.setTitle("Roll")
    embed.addField("You rolled a "+rollTotal, "and landed on "+DiscordMonies.Players[user.id].Game.Board.Fields[DiscordMonies.Players[user.id].Position].name)
    embed.setFooter(roll + " + " + roll2 + " = " + rollTotal)
    msgo.channel.send({embed:embed})
